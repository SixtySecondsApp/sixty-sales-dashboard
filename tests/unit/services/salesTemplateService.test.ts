...
import { SalesTemplateService, SalesTemplate, TemplateContext, TemplateCategory } from '@/lib/services/salesTemplateService';
...
vi.mock('@/lib/services/linkedinEnrichmentService', () => ({
  LinkedInEnrichmentService: {
    findLinkedInUrl: vi.fn(),
    scrapeProfile: vi.fn(),
  }
}));
...
describe('SalesTemplateService', () => {
  ...
  describe('getTemplates', () => {
    it('should fetch active templates for the user', async () => {
      ...
      const mockTemplateChain = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        or: vi.fn().mockReturnThis(),
        order: vi.fn().mockReturnThis(),
        then: (resolve: any) => resolve({ data: [mockTemplate], error: null }), // Make it thenable
      };

      vi.mocked(supabase.from).mockImplementation((table: string) => {
        if (table === 'profiles') return mockProfileChain as any;
        if (table === 'sales_templates') return mockTemplateChain as any;
        return {} as any;
      });
  ...
  describe('personalizeTemplate', () => {
    ...
    it('should perform external enrichment when enabled', async () => {
      // Mock template fetching
      const mockSelect = vi.fn().mockReturnThis();
      const mockEq = vi.fn().mockReturnThis();
      const mockSingle = vi.fn().mockResolvedValue({ data: mockTemplate, error: null });
      vi.mocked(supabase.from).mockReturnValue({ select: mockSelect, eq: mockEq, single: mockSingle } as any);

      // Mock LinkedIn Service
      vi.mocked(LinkedInEnrichmentService.findLinkedInUrl).mockResolvedValue('https://linkedin.com/in/john-doe');
      vi.mocked(LinkedInEnrichmentService.scrapeProfile).mockResolvedValue({
      ...
      // Mock Copilot responses (Smart Context & Email Gen)
      vi.mocked(CopilotService.sendMessage)
        .mockResolvedValueOnce({ // Smart Context Response
          response: { content: '{"pain_points": ["Scaling"], "value_propositions": ["Automation"], "recent_news": [], "ice_breakers": ["Saw your post"]}' }
        } as any)
        .mockResolvedValueOnce({ // Email Gen Response
          response: { content: 'SUBJECT: AI Subject\n\nAI Body' }
        } as any);

      const result = await SalesTemplateService.personalizeTemplate(
        'template-1', 
        mockContext, 
        { skipAI: false, enrichContext: true }
      );

      expect(LinkedInEnrichmentService.findLinkedInUrl).toHaveBeenCalled();
      expect(LinkedInEnrichmentService.scrapeProfile).toHaveBeenCalled();
      expect(result.smart_context_used).toBe(true);
    });
