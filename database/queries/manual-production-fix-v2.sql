-- API Keys System Production Fix V2
-- Run this in Supabase Dashboard > SQL Editor
-- This version handles existing functions and table conflicts
...
-- Drop and recreate RLS policies for api_requests
...
-- Step 8: Create api_key_usage table for rate limiting (if not exists)
...
-- Drop and recreate policies for api_key_usage
DROP POLICY IF EXISTS "Service role can insert usage" ON api_key_usage;
...
SELECT 
    '✅ API Keys database setup completed!' as status,
    'Please test API key creation using the test page' as next_step;