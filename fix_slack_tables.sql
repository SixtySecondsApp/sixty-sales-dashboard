...
DROP POLICY IF EXISTS "Users can view own Slack channels" ON slack_channels;
DROP POLICY IF EXISTS "Users can insert own Slack channels" ON slack_channels;
DROP POLICY IF EXISTS "Users can update own Slack channels" ON slack_channels;
DROP POLICY IF EXISTS "Users can delete own Slack channels" ON slack_channels;
DROP POLICY IF EXISTS "Service role can manage all Slack channels" ON slack_channels;

-- Create comprehensive RLS policies for slack_integrations
...
-- Create comprehensive RLS policies for slack_channels  
...
CREATE POLICY "Users can insert own Slack channels" ON slack_channels
...
CREATE POLICY "Users can update own Slack channels" ON slack_channels
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM slack_integrations
      WHERE slack_integrations.id = slack_channels.integration_id
...
CREATE POLICY "Users can delete own Slack channels" ON slack_channels
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM slack_integrations
      WHERE slack_integrations.id = slack_channels.integration_id
