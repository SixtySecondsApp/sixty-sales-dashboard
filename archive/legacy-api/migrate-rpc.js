...
export default async function handler(request, response) {
  ...
  if (request.method !== 'POST') {
    return apiResponse(response, { error: 'Method not allowed' }, 405);
  }

  try {
    console.log('Creating get_users_with_targets RPC function...');
    ...
    return apiResponse(response, {
      success: true,
      message: 'RPC function get_users_with_targets created successfully'
    });
