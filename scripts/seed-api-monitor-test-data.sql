-- Seed test data for API Monitor
-- This creates some sample rollup data so the UI has something to display
...
INSERT INTO api_monitor_snapshots (
  ...
  '[
    {"endpoint": "/rest/v1/user_notifications", "requests_per_minute": 75, "time_window": "' || (NOW() - INTERVAL '30 minutes')::text || '"}
  ]'::jsonb,
  'supabase_logs'
...
-- Insert a test daily rollup for a specific user (if you have a user ID)
-- This will be populated automatically by the app instrumentation, but we can seed one
-- Replace with an actual user ID from your profiles table if you want
-- INSERT INTO api_monitor_rollups_daily (
--   date,
--   user_id,
--   total_requests,
...
--   15,
--   3.0,
--   '[{"endpoint": "/rest/v1/deals", "method": "GET", "count": 200}]'::jsonb,
--   '{"400": 10, "500": 5}'::jsonb
