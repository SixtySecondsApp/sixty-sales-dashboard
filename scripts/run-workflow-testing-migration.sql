...
-- Complete migration with proper table references

-- Ensure user_profiles exists (from previous migration)
...
CREATE TABLE IF NOT EXISTS workflow_environment_promotions (
  ...
  promoted_by UUID,  -- Removed reference to user_profiles
...
-- Create unique indexes for proper constraint handling
-- For variable_storage: ensure unique keys per scope and workflow
...
-- Apply updated_at triggers (DROP and CREATE to avoid errors)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Advanced Workflow Testing System migration completed successfully!';
  RAISE NOTICE '';
  RAISE NOTICE 'Tables created:';
  RAISE NOTICE '  • workflow_environments - Build/Staging/Live environments';
  RAISE NOTICE '  • workflow_contracts - JSON Schema type definitions';
  RAISE NOTICE '  • execution_snapshots - Time-travel debugging';
  RAISE NOTICE '  • node_fixtures - Pinned inputs and golden outputs';
  RAISE NOTICE '  • scenario_fixtures - End-to-end test scenarios';
  RAISE NOTICE '  • variable_storage - Scoped variable system';
  RAISE NOTICE '  • execution_checkpoints - Resume points';
  RAISE NOTICE '  • http_request_recordings - API replay data';
  RAISE NOTICE '  • workflow_idempotency_keys - Duplicate prevention';
  RAISE NOTICE '  • workflow_dead_letter_queue - Failed execution handling';
  RAISE NOTICE '  • workflow_rate_limits - Rate limiting';
  RAISE NOTICE '  • workflow_circuit_breakers - Failure protection';
  RAISE NOTICE '  • workflow_batch_windows - Batch processing';
  RAISE NOTICE '  • workflow_environment_promotions - Environment history';
  RAISE NOTICE '  • webhook_mirror_config - Traffic mirroring';
END $$;