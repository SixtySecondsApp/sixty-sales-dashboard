...
-- Complete version with all tables including tasks
-- ================================================================
...
BEGIN
    ...
    -- Step 7: Update tasks table (assigned_to and created_by)
...
SELECT
    'tasks (assigned_to)' as table_name,
    COUNT(*) as total_records,
    COUNT(DISTINCT t.assigned_to) as unique_users,
    SUM(CASE WHEN p.id IS NOT NULL THEN 1 ELSE 0 END) as linked_to_current_profiles,
    SUM(CASE WHEN p.id IS NULL THEN 1 ELSE 0 END) as orphaned_records
FROM tasks t
