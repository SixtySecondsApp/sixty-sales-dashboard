-- ============================================================================
-- COMPREHENSIVE Stage Cleanup - Remove Test Stages Safely
-- ============================================================================
...
SELECT 
  ...
  (SELECT COUNT(*) FROM deal_stage_history WHERE stage_id = deal_stages.id) as history_entries
FROM deal_stages
LEFT JOIN deals ON deal_stages.id = deals.stage_id
WHERE name IN ('Lead', 'Qualified', 'Closed Won')
GROUP BY deal_stages.id, deal_stages.name;
