-- Safe version of the merge script that temporarily disables audit triggers
-- This prevents duplicate audit log entries during the merge process
...
-- STEP 2: Temporarily disable audit triggers to prevent conflicts
SELECT '=== STEP 2: Disabling audit triggers ===' as step;
ALTER TABLE companies DISABLE TRIGGER audit_companies_trigger;

-- STEP 3: Undo the renaming by removing the (2), (3), etc. suffixes
SELECT '=== STEP 3: Undoing renaming ===' as step;
UPDATE companies
...
-- STEP 4: Show what will be merged (for transparency)
SELECT '=== STEP 4: Companies to be merged ===' as step;
SELECT 
    name,
...
SELECT '=== STEP 5: Merging duplicates ===' as step;
DO $$
DECLARE
    v_dup_group RECORD;
...
SELECT '=== STEP 6: Re-enabling audit triggers ===' as step;
ALTER TABLE companies ENABLE TRIGGER audit_companies_trigger;
...
-- STEP 8: Show the final merged companies
SELECT '=== STEP 8: Final merged companies ===' as step;
SELECT id, name, created_at, owner_id
FROM companies
WHERE name IN ('Aleto Foundation', 'Fynity Talent', 'Happiest at Home', 'Nicer Group', 'Pure Performance')
ORDER BY name;

-- STEP 9: Show count of related records for merged companies
SELECT '=== STEP 9: Related records count ===' as step;
SELECT 
...
-- STEP 10: Ready for main migration
SELECT '=== STEP 10: Next Steps ===' as step;
