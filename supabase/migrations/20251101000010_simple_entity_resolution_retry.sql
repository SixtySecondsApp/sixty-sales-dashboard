-- Simple Entity Resolution Retry
-- This is a simplified version that just runs the migration with SECURITY DEFINER
-- No complex rollback logic - just clean slate and retry

-- Step 1: Clean slate (simple)
TRUNCATE TABLE deal_migration_reviews CASCADE;

-- Clear ALL entity relationships from deals (we'll recreate them)
UPDATE deals
SET company_id = NULL,
    primary_contact_id = NULL,
    updated_at = NOW()
WHERE company_id IS NOT NULL OR primary_contact_id IS NOT NULL;

-- Step 2: Ensure functions have SECURITY DEFINER
...
-- Create migrate function with SECURITY DEFINER
...
-- Create resolve function with SECURITY DEFINER
...
BEGIN
  RAISE NOTICE 'Starting entity resolution migration with SECURITY DEFINER...';
