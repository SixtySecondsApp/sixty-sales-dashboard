-- CORRECTED VERSION: This migration properly handles the meetings table column names
...
CREATE POLICY "Users can view action items for their meetings"
  ...
  USING (
    EXISTS (
      ...
      AND meetings.owner_user_id = auth.uid()  -- CORRECTED: owner_user_id not user_id
    )
  );
...
-- Add helpful comments explaining the column purposes and naming
...
COMMENT ON COLUMN meetings.owner_user_id IS 'IMPORTANT: This is the correct column name for the meeting owner (NOT user_id)';
