...
-- Indexes
CREATE INDEX idx_pipeline_recommendations_meeting ON pipeline_stage_recommendations(meeting_id);
CREATE INDEX idx_pipeline_recommendations_deal ON pipeline_stage_recommendations(deal_id);
CREATE INDEX idx_pipeline_recommendations_company ON pipeline_stage_recommendations(company_id);
CREATE INDEX idx_pipeline_recommendations_user ON pipeline_stage_recommendations(user_id);
CREATE INDEX idx_pipeline_recommendations_status ON pipeline_stage_recommendations(status) WHERE status = 'pending';
CREATE INDEX idx_pipeline_recommendations_created ON pipeline_stage_recommendations(created_at DESC);
...
BEGIN
  ...
  GET DIAGNOSTICS v_deal_updated = ROW_COUNT > 0;

  IF v_deal_updated THEN
...
-- Trigger to auto-generate recommendations when meetings are synced
CREATE OR REPLACE FUNCTION trigger_pipeline_recommendation_on_meeting_sync()
RETURNS TRIGGER AS $$
BEGIN
  -- Only generate recommendations for synced meetings with sentiment scores
  IF NEW.sync_status = 'synced' AND NEW.sentiment_score IS NOT NULL AND NEW.company_id IS NOT NULL THEN
    -- Generate recommendation (will return NULL if no active deal or no change needed)
    PERFORM generate_pipeline_recommendation_from_meeting(NEW.id, NEW.owner_user_id);
  END IF;
...
DROP TRIGGER IF EXISTS generate_pipeline_recommendation_on_sync ON meetings;
CREATE TRIGGER generate_pipeline_recommendation_on_sync
AFTER INSERT OR UPDATE ON meetings
FOR EACH ROW
WHEN (NEW.sync_status = 'synced' AND NEW.sentiment_score IS NOT NULL)
EXECUTE FUNCTION trigger_pipeline_recommendation_on_meeting_sync();

-- Comments
COMMENT ON FUNCTION generate_pipeline_recommendation_from_meeting IS 'Analyzes meeting sentiment and generates pipeline stage recommendation';
COMMENT ON FUNCTION approve_pipeline_recommendation IS 'Approves recommendation and updates deal stage (HITL approval)';
COMMENT ON FUNCTION reject_pipeline_recommendation IS 'Rejects recommendation without changing deal stage';
