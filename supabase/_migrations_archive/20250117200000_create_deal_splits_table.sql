/*
  ...
  NOTE: Made conditional for staging compatibility where deals/profiles
  tables may not exist yet.
*/

-- Only create deal_splits if both deals and profiles tables exist
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'deals')
    ...
    -- Create deal_splits table without the problematic GENERATED column
    -- (PostgreSQL doesn't allow subqueries in GENERATED columns)
    -- The amount will be calculated via trigger instead
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'deal_splits') THEN
      ...
      RAISE NOTICE 'Created deal_splits table';
    END IF;
    ...
    RAISE NOTICE 'Deal splits migration completed successfully';
  ELSE
    RAISE NOTICE 'Skipping deal_splits migration - deals or profiles table does not exist';
  END IF;
END $$;
