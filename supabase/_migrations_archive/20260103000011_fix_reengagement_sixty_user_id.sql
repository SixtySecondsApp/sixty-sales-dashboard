-- Fix get_reengagement_candidates function to use sixty_user_id column
-- The slack_user_mappings table uses sixty_user_id (not user_id) to reference auth.users
...
-- Also fix the get_notification_candidates_for_testing function
...
COMMENT ON FUNCTION get_reengagement_candidates IS 'Get users eligible for re-engagement - uses sixty_user_id from slack_user_mappings';
