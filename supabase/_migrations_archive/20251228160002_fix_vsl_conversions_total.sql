-- Fix VSL Analytics: Show TOTAL conversions per variant, not just date-matched
-- This ensures historic signups are counted regardless of when video was watched

-- Drop existing view
DROP VIEW IF EXISTS public.vsl_analytics_summary;

-- Recreate view with total conversions per variant
...
-- Count TOTAL waitlist signups by variant (for overall conversion tracking)
...
-- Pre-aggregate watch time stats
watch_time_stats AS (
  SELECT
    signup_source,
    video_public_id,
...
SELECT
  ...
  -- Daily conversions for trend tracking
  COALESCE(dc.conversion_count, 0) as daily_conversions,
  -- TOTAL conversions for this variant (same value across all dates)
...
COMMENT ON VIEW public.vsl_analytics_summary IS
  'VSL video analytics with watch time from all events and conversion tracking. conversions = total for variant, daily_conversions = per-day count.';
