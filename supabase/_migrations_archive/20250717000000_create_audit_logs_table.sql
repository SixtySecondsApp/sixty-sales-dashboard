...
-- This migration creates the base table that subsequent migrations will modify
-- IMPORTANT: This must run before 20250717162134_add_session_id_to_audit_logs.sql
...
-- Create indexes for efficient querying (IF NOT EXISTS for safety)
...
-- Allow insert for authenticated users (triggers need this)
DROP POLICY IF EXISTS "Allow audit log inserts" ON audit_logs;
CREATE POLICY "Allow audit log inserts" ON audit_logs
  FOR INSERT
  WITH CHECK (true);
...
-- Basic audit trigger function (will be updated by subsequent migrations)
...
-- Create audit triggers for important tables (safe to re-run)
