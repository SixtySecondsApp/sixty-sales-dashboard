-- Update the design_system template with the latest content using dollar quoting to handle single quotes safely

UPDATE proposal_templates
SET content = $template$# Universal Design System
...
| Danger | `red-600` | `red-500` |$template$,
    updated_at = NOW()
WHERE type = 'design_system' AND is_default = true;

-- Insert if it doesnt exist (for safety)
INSERT INTO proposal_templates (type, content, is_default)
SELECT 'design_system', $template$# Universal Design System
## Clean Light Mode + Premium Glassmorphic Dark Mode
...
| Primary Color | `blue-600` | `blue-400` |
| Success | `emerald-600` | `emerald-500` |
| Danger | `red-600` | `red-500` |$template$, true
WHERE NOT EXISTS (
