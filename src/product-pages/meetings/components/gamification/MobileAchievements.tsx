  ...
  const achievements: Achievement[] = [
    {
      ...
      unlockCondition: 'Copy & share now!',
      icon: <Share2 className="w-8 h-8" />,
      unlocked: hasShared,
      progress: hasShared ? 100 : 0,
      progressText: hasShared ? '✓ Done' : 'Start now',
      color: 'from-blue-500 to-blue-600'
    },
    {
      type: 'linkedin_boost',
      title: 'LinkedIn Pro',
      description: '+50 LinkedIn boost',
      unlockCondition: 'Post on LinkedIn!',
    ...
    },
    {
      type: 'twitter_boost',
      title: 'X Poster',
      description: '+50 Twitter/X boost',
      unlockCondition: 'Post on X!',
    ...
    },
    {
      type: 'email_boost',
      title: 'Email Sharer',
      description: '+50 Email boost',
      unlockCondition: 'Share via Email!',
      icon: <Share2 className="w-8 h-8" />,
      unlocked: emailBoostClaimed,
      progress: emailBoostClaimed ? 100 : 0,
      progressText: emailBoostClaimed ? '+50!' : 'Get boost',
    ...
    },
    {
      ...
      title: 'Rising Star',
      description: 'Top 100',
      unlockCondition: effectivePosition > 100 ? `${effectivePosition - 100} pts!` : 'Keep going!',
    ...
    },
    {
      ...
      unlockCondition: effectivePosition > 50 ? `${effectivePosition - 50} pts!` : 'VIP!',
      ...
      progressText: effectivePosition <= 50 ? 'VIP!' : `#${formatRank(effectivePosition)}`,
    ...
    >
      ...
      <div className="relative overflow-hidden">
        <motion.div
          drag="x"
          dragConstraints={{
            left: -(achievements.length - 1) * 148,
            right: 0
          }}
          dragElastic={0.1}
          dragMomentum={false}
          className="flex gap-3 cursor-grab active:cursor-grabbing pb-4"
        >
          {achievements.map((achievement) => (
            <motion.div
              key={achievement.type}
              className={`
                flex-shrink-0 w-32 rounded-xl p-3 text-center select-none transition-all flex flex-col
            ...
            >
              {/* Icon */}
              <div className={`
                w-12 h-12 mx-auto mb-2 rounded-full flex items-center justify-center flex-shrink-0
                ${achievement.unlocked
              ...
              {/* Title */}
              <h4 className={`font-bold text-xs mb-1 ${achievement.unlocked ? 'text-white' : 'text-gray-300'}`}>
                {achievement.title}
              </h4>

              {/* Description or Unlock Condition */}
              <p className={`text-[10px] leading-snug flex-grow ${achievement.unlocked ? 'text-white/90' : 'text-gray-400'}`}>
              ...
              {achievement.unlocked && (
                <div className="mt-1.5">
                  <span className="inline-block px-1.5 py-0.5 bg-white/20 text-white text-[9px] font-bold rounded-full">
                    {achievement.progressText}
                  </span>
                </div>
              )}
