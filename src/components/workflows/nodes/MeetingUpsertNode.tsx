      ...
      <div className="p-3 space-y-3 bg-white dark:bg-[#1e1e1e]">
        {data.upsertKey && (
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-gray-500 dark:text-zinc-500 uppercase tracking-wider">Upsert Key</label>
            <div className="text-xs text-gray-700 dark:text-zinc-300 bg-gray-50 dark:bg-zinc-900/50 p-2 rounded border border-gray-200 dark:border-zinc-800 font-mono">
              {data.upsertKey}
            </div>
          </div>
        )}
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              {enabledFeatures.map((feature, idx) => (
                <div key={idx} className="flex items-center gap-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-500/10 rounded border border-blue-200 dark:border-blue-500/20 text-[9px] text-blue-700 dark:text-blue-300">
                  <Check size={8} />
                  {feature}
                </div>
              ))}
