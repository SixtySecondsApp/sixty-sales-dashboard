/**
 * Fathom Integration Tests Page
 *
 * Individual integration test page for Fathom.
...
import { runTestSuite, runSingleTest, getTestHistory, getLatestTestResults } from '@/lib/integrationTesting/testRunner';
...
function HistoryItem({ record }: HistoryItemProps) {
  ...
  const { bg, text, icon: StatusIcon } = statusStyles[status] || statusStyles.pending;
...
export default function FathomIntegrationTests() {
    ...
    const fetchData = async () => {
      try {
        const [status, testHistory, latestResults] = await Promise.all([
          getFathomConnectionStatus(activeOrgId),
          getTestHistory('fathom', 20),
          getLatestTestResults('fathom'),
        ]);

        setConnectionStatus(status);
        setTests(createFathomTests(activeOrgId));
        ...
        console.error('[FathomIntegrationTests] Error fetching data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      for (const test of tests) {
        ...
        const result = await runSingleTest('fathom', test, 'manual', activeOrgId);
      ...
      // Refresh history
      const testHistory = await getTestHistory('fathom', 20);
      setHistory(testHistory);
      ...
      console.error('[FathomIntegrationTests] Error running tests:', error);
  ...
  const handleRunTest = async (test: IntegrationTest) => {
    ...
    try {
      ...
      console.error('[FathomIntegrationTests] Error running test:', error);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Tests by Category */}
        {['authentication', 'connectivity', 'sync', 'data', 'webhook', 'infrastructure'].map((category) => {
        ...
        >
          <div>
            ...
            <p className="text-sm text-blue-600 dark:text-blue-300">
              Check the Fathom integration documentation for common issues.
            </p>
          </div>
          <a
            href="https://fathom.video/docs"
            target="_blank"
