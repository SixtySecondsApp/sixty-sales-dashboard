/**
 * SavvyCal Integration Tests Page
 *
 * Individual integration test page for SavvyCal.
 * Allows running diagnostic tests and viewing test history.
 */
...
export default function SavvyCalIntegrationTests() {
    ...
    const fetchData = async () => {
      try {
        const [status, testHistory, latestResults] = await Promise.all([
          getSavvyCalConnectionStatus(activeOrgId),
          getTestHistory('savvycal', 20),
          getLatestTestResults('savvycal'),
        ]);

        setConnectionStatus(status);
        setTests(createSavvyCalTests(activeOrgId));
        ...
        console.error('[SavvyCalIntegrationTests] Error fetching data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      for (const test of tests) {
        ...
        const result = await runSingleTest('savvycal', test, 'manual', activeOrgId);
      ...
      // Refresh history
      const testHistory = await getTestHistory('savvycal', 20);
      setHistory(testHistory);
      ...
      console.error('[SavvyCalIntegrationTests] Error running tests:', error);
  ...
  const handleRunTest = async (test: IntegrationTest) => {
    ...
    try {
      ...
      console.error('[SavvyCalIntegrationTests] Error running test:', error);
  ...
  const handleCopyResultsForAI = async () => {
    ...
    const lines: string[] = [
      '# SavvyCal Integration Test Results',
      '',
      `**Date:** ${new Date().toLocaleString()}`,
      `**Connection Status:** ${connectionStatus?.isConnected ? 'Connected' : 'Not Connected'}`,
      connectionStatus?.accountInfo?.webhookConfigured ? '**Webhook:** Configured' : '',
      '',
      '## Summary',
      `- Total: ${summary.total}`,
      ...
      <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    SavvyCal Integration Tests
                  </h1>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Scheduling and lead capture
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {connectionStatus?.isConnected && connectionStatus.accountInfo?.webhookConfigured && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Webhook configured
                  </p>
                )}
        ...
        <div
          className={cn(
            ...
            'border border-cyan-200 dark:border-cyan-500/20 rounded-xl p-4',
            'flex items-center justify-between'
          )}
        >
          <div>
            <p className="font-medium text-cyan-700 dark:text-cyan-400">
              Configure SavvyCal Settings
            </p>
            <p className="text-sm text-cyan-600 dark:text-cyan-300">
              Manage API token, webhook, and source mappings.
            </p>
          </div>
          <Link
            to="/platform/integrations/savvycal"
