/**
 * HubSpot Integration Tests Page
 *
 * Individual integration test page for HubSpot.
...
export default function HubSpotIntegrationTests() {
    ...
    const fetchData = async () => {
      try {
        const [status, testHistory, latestResults] = await Promise.all([
          getHubSpotConnectionStatus(activeOrgId),
          getTestHistory('hubspot', 20),
          getLatestTestResults('hubspot'),
        ]);

        setConnectionStatus(status);
        setTests(createHubSpotTests(activeOrgId));
        ...
        console.error('[HubSpotIntegrationTests] Error fetching data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      for (const test of tests) {
        ...
        const result = await runSingleTest('hubspot', test, 'manual', activeOrgId);
      ...
      // Refresh history
      const testHistory = await getTestHistory('hubspot', 20);
      setHistory(testHistory);
      ...
      console.error('[HubSpotIntegrationTests] Error running tests:', error);
  ...
  const handleRunTest = async (test: IntegrationTest) => {
    ...
    try {
      ...
      console.error('[HubSpotIntegrationTests] Error running test:', error);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {connectionStatus?.isConnected && connectionStatus.accountInfo?.name && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Portal: {connectionStatus.accountInfo.name} ({connectionStatus.accountInfo.id})
                  </p>
                )}
        ...
        >
          <div>
            ...
            <p className="text-sm text-blue-600 dark:text-blue-300">
              Manage sync options, pipeline mapping, and feature settings.
            </p>
          </div>
          <Link
