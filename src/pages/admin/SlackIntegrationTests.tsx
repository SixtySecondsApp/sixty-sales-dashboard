/**
 * Slack Integration Tests Page
 *
 * Individual integration test page for Slack.
...
export default function SlackIntegrationTests() {
    ...
    const fetchData = async () => {
      try {
        const [status, testHistory, latestResults] = await Promise.all([
          getSlackConnectionStatus(activeOrgId),
          getTestHistory('slack', 20),
          getLatestTestResults('slack'),
        ]);

        setConnectionStatus(status);
        setTests(createSlackTests(activeOrgId));
        ...
        console.error('[SlackIntegrationTests] Error fetching data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRunAll = async () => {
    ...
    try {
      ...
      for (const test of tests) {
        ...
        const result = await runSingleTest('slack', test, 'manual', activeOrgId);
      ...
      // Refresh history
      const testHistory = await getTestHistory('slack', 20);
      setHistory(testHistory);
      ...
      console.error('[SlackIntegrationTests] Error running tests:', error);
  ...
  const handleRunTest = async (test: IntegrationTest) => {
    ...
    try {
      ...
      console.error('[SlackIntegrationTests] Error running test:', error);
  ...
  const handleCopyResultsForAI = async () => {
    ...
    const lines: string[] = [
      ...
      connectionStatus?.accountInfo?.name ? `**Workspace:** ${connectionStatus.accountInfo.name}` : '',
      '',
      '## Summary',
      `- Total: ${summary.total}`,
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {connectionStatus?.isConnected && connectionStatus.accountInfo?.name && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Workspace: {connectionStatus.accountInfo.name}
                  </p>
                )}
        ...
        <div
          className={cn(
            ...
            'border border-purple-200 dark:border-purple-500/20 rounded-xl p-4',
        ...
        >
          <div>
            ...
            <p className="text-sm text-purple-600 dark:text-purple-300">
              Manage notification features, channels, and user mappings.
            </p>
          </div>
          <Link
