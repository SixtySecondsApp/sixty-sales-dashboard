...
export default function BrandingSettings() {
  ...
  const updateFavicon = useCallback((iconUrl: string) => {
    ...
    // Create new favicon links for different sizes
  ...
  // Update favicon when icon_url changes
  useEffect(() => {
    ...
    try {
      ...
      if (data) {
        ...
        const { data: newData, error: createError } = await supabase
          .from('branding_settings')
          .upsert(
            { id: defaultId, logo_light_url: null, logo_dark_url: null, icon_url: null },
            { onConflict: 'id' }
          )
          .select()
          .single();
      ...
      logger.error('[BrandingSettings] Load error:', error);
  ...
  const handleFileUpload = async (
    ...
    // Validate file size (2MB max for logos, 500KB for icon)
    ...
    try {
      ...
      // If icon was updated, update favicon
      if (type === 'icon') {
      ...
      logger.error('[BrandingSettings] Upload error:', error);
  ...
  const handleRemove = async (type: 'lightLogo' | 'darkLogo' | 'icon') => {
    ...
    try {
      ...
      // If icon was removed, reset to default favicon
      if (type === 'icon') {
      ...
      logger.error('[BrandingSettings] Remove error:', error);
      ...
      <div className="p-6">
        <Card>
          <CardContent className="pt-6">
            <p className="text-center text-gray-500">Admin access required to manage branding settings.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Branding & Customization</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Customize your application logos and favicon
          </p>
        </div>
        ...
        <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
          ...
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Logos are uploaded to S3 in the <code className="px-1.5 py-0.5 bg-white dark:bg-gray-800 rounded text-xs">erg logos</code> folder. 
              Make sure AWS credentials are configured in your Supabase Edge Function secrets.
            </p>
          </CardContent>
        </Card>
      </div>
